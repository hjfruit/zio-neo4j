...
final class Neo4jTransactionLive(underlying: AsyncTransaction) extends Neo4jTransaction:
  ...
  override def commit: Task[Unit] = ZIO.fromCompletionStage(underlying.commitAsync()).unit

  override def rollback: Task[Unit] = ZIO.fromCompletionStage(underlying.rollbackAsync()).unit
